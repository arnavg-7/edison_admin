...
type Tab = "enrollment" | "academic";

/**
 * Reference numbers, one grouped cluster at a time, so the eight cards this
 * used to be a flat grid of don't all compete for attention at once. Needs
 * Attention already led the page above this — these are the steady-state
 * figures a Super Admin checks second.
 */
export function HomeMetricTabs() {
  const [tab, setTab] = useState<Tab>("enrollment");

  return (
    <div className="sf-home-metrics">
      <div className="sf-tabs" role="tablist" aria-label="Home metric groups">
        <button
          type="button"
          role="tab"
          aria-selected={tab === "enrollment"}
          className={tab === "enrollment" ? "sf-tab active" : "sf-tab"}
        ...
        <button
          ...
          className={tab === "academic" ? "sf-tab active" : "sf-tab"}
      ...
      {tab === "enrollment" ? (
        <div className="sf-card-grid" role="tabpanel">
          <MetricCard
